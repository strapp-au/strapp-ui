    ...
    return object : FrameHandler {
      ...
      override fun close() {
        ...
        val shot = if (hashes.size == 1) {
          ...
          if (isRecording) {
            val goldenFile = File(snapshotRootDirectory, snapshot.toFileName("_", "png"))
          ...
          if (isRecording) {
            for ((index, frameHash) in hashes.withIndex()) {
              val originalFrame = File(snapshotRootDirectory, "$frameHash.png")
              val frameSnapshot = snapshot.copy(name = "${snapshot.name} $index")
              val goldenFile = File(snapshotRootDirectory, frameSnapshot.toFileName("_", "png"))
